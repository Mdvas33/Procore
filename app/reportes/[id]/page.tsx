...
function VerticalChart({ items }: { items: { label: string; value: number }[] }) {
  ...
  return <div className="grid grid-cols-[28px_1fr] gap-3"><div className="flex h-80 flex-col justify-between pb-12 text-right text-xs text-zinc-600">{axisSteps.map((step) => <span key={step}>{step}</span>)}</div><div className="relative h-80 border-b border-zinc-300"><div className="absolute inset-x-0 top-0 flex h-64 flex-col justify-between">{axisSteps.map((step) => <span key={step} className="border-t border-zinc-200" />)}</div><div className="relative flex h-64 items-end justify-around gap-2 px-2">{items.map((item) => <div key={item.label} className="flex h-full flex-1 items-end justify-center"><div className="relative w-full max-w-28 rounded-t-sm bg-[#5795cc]" style={{ height: `${Math.max((item.value / max) * 100, 4)}%` }}><strong className="absolute -top-6 left-0 w-full text-center text-sm font-semibold text-zinc-800">{item.value}</strong></div></div>)}</div><div className="flex h-16 items-start justify-around gap-2 px-2 pt-2">{items.map((item) => <div key={item.label} className="min-w-0 flex-1 text-center text-xs text-zinc-700"><span className="block truncate" title={item.label}>{item.label}</span></div>)}</div></div></div>;
}
