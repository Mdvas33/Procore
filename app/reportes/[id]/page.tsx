...
function VerticalChart({ items, legendLabel = "Inspecciones" }: { items: { label: string; value: number }[]; legendLabel?: string }) {
  ...
  return (
    <div className="grid grid-cols-[42px_1fr] gap-3 overflow-visible">
      <div className="relative flex h-[420px] flex-col justify-between pb-16 pr-1 text-right text-xs text-zinc-600">
        <span className="absolute -left-5 top-1/2 -translate-y-1/2 -rotate-90 text-xs font-medium tracking-wide text-zinc-700">CUENTA</span>
        {axisSteps.map((step) => <span key={step}>{step}</span>)}
      </div>
      <div className="relative h-[420px] overflow-visible">
        <div className="absolute inset-x-0 top-0 flex h-[310px] flex-col justify-between">
          {axisSteps.map((step) => <span key={step} className="border-t border-zinc-200" />)}
        </div>
        <div className="relative flex h-[310px] items-end justify-around gap-2 px-3 pt-2">
          {items.map((item) => (
            <div key={item.label} className="flex h-full flex-1 items-end justify-center">
              <div className="relative w-full max-w-28 rounded-t-sm" style={{ height: `${Math.max((item.value / chartMax) * 100, 1.5)}%`, backgroundColor: "#5795cc" }} />
            </div>
          ))}
        </div>
